<?php
     ...
     class Objet_models extends CI_Model { 
          public function modify_objet($nom , $qte , $price , $user , $categorie , $id){
          ...
          public function add_objet($nom , $qte , $price , $user , $categorie){
               $request = "INSERT INTO objets values (null,'%s','%s','%s','%s') ";
               $request = sprintf($request,$this->db->escape($nom),
                                        $this->db->escape($qte),
                                        $this->db->escape($price),
                                        $this->db->escape($user),
          ...
          public function getListOfObjet($id){
               ...
               echo $id ; 
               echo $request;
               $request = $request = sprintf($request,$this->db->escape($id));
          ...
          public function getOneObjet($id){
               $request = 'SELECT id_categories FROM objets where id_utilisateurs = %s  ';
               $request = $request = sprintf($request,$this->db->escape($id));
               $query = $this->db->query($request);
          ...
          public function getMyObjet($id){
               $request = 'SELECT * FROM objets where id_utilisateurs = %s  ';
               $request = $request = sprintf($request,$this->db->escape($id));
               $query = $this->db->query($request);
               $result = array();
               foreach ($query -> result_array() as $row ){
